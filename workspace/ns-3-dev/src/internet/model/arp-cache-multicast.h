 ...
 */
#ifndef ARP_CACHE_MULTICAST_H
#define ARP_CACHE_MULTICAST_H
 ...
 */
class ArpCacheMulticast : public Object
{
private:
  /**
   ...
   */
  ArpCacheMulticast (ArpCacheMulticast const &);
  /**
   * \brief Copy constructor
   *
   * Defined and unimplemented to avoid misuse
   * \returns
   */
  ArpCacheMulticast& operator= (ArpCacheMulticast const &);
...
public:
  ...
  /**
   * \brief Set the NetDevice and Ipv4InterfaceMulticast associated with the ArpCacheMulticast
   *
   * \param device The hardware NetDevice associated with this ARP cache
   * \param interface the Ipv4InterfaceMulticast associated with this ARP cache
   */
  void SetDevice (Ptr<NetDevice> device, Ptr<Ipv4InterfaceMulticast> interface);
  /**
  ...
  /**
   * \brief Returns the Ipv4InterfaceMulticast that this ARP cache is associated with
   * \return the Ipv4InterfaceMulticast that this ARP cache is associated with
   */
  ...
  /**
   * This callback is set when the ArpCacheMulticast is set up and allows
   ...
   */
  void SetArpRequestCallback (Callback<void, Ptr<const ArpCacheMulticast>, 
   ...
   * of
   * \return An ArpCacheMulticast::Entry with info about layer 2
   */
  ArpCacheMulticast::Entry *Lookup (Ipv4Address destination);
  /**
   * \brief Add an Ipv4Address to this ARP cache
   */
  ArpCacheMulticast::Entry *Add (Ipv4Address to);
  /**
   * \brief Remove an entry.
   * \param entry pointer to delete it from the list
   */
  void Remove (ArpCacheMulticast::Entry *entry);
  /**
   * \brief Clear the ArpCacheMulticast of all entries
   */
  void Flush (void);
  ...
  /**
   * \brief A record that that holds information about an ArpCacheMulticast entry
   */
  class Entry {
public:
    /**
     * \brief Constructor
     * \param arp The ArpCacheMulticast this entry belongs to
     */
    Entry (ArpCacheMulticast *arp);
...
private:
    ...
    ArpCacheMulticast *m_arp; //!< pointer to the ARP cache owning the entry
...
private:
   ...
   */
  typedef sgi::hash_map<Ipv4Address, ArpCacheMulticast::Entry *, Ipv4AddressHash> Cache;
  /**
   * \brief ARP Cache container iterator
   */
  typedef sgi::hash_map<Ipv4Address, ArpCacheMulticast::Entry *, Ipv4AddressHash>::iterator CacheI;

  virtual void DoDispose (void);

  Ptr<NetDevice> m_device; //!< NetDevice associated with the cache
  Ptr<Ipv4InterfaceMulticast> m_interface; //!< Ipv4InterfaceMulticast associated with the cache
  ...
  Callback<void, Ptr<const ArpCacheMulticast>, Ipv4Address> m_arpRequestCallback;  //!< reply timeout callback
  uint32_t m_maxRetries; //!< max retries for a resolution

  /**
   * This function is an event handler for the event that the
   * ArpCacheMulticast wants to check whether it must retry any Arp requests.
