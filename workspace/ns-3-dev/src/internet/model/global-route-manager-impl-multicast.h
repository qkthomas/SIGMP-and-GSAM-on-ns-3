...
#ifndef GLOBAL_ROUTE_MANAGER_IMPL_MULTICAST_H
#define GLOBAL_ROUTE_MANAGER_IMPL_MULTICAST_H
...
const uint32_t SPF_INFINITY_MULTICAST = 0xffffffff; //!< "infinite" distance between nodes

class CandidateQueueMulticast;
class Ipv4GlobalRoutingMulticast;

/**
 * @brief Vertex used in shortest path first (SPF) computations. See \RFC{2328},
 * Section 16.
 *
 * Each router in the simulation is associated with an SPFVertexMulticast object.  When
 ...
 * routers in the calculation has an associated SPFVertexMulticast.
 *
 * The "Root" vertex is the SPFVertexMulticast representing the router that is having
 * its routing tables set.  The SPFVertexMulticast objects representing other routers
 ...
 *
 * Each SPFVertexMulticast has a pointer to the Global Router Link State Advertisement
 ...
 * underlying router to other nodes (represented by other SPFVertexMulticast objects)
 * in the simulation topology.  The combination of the arrangement of the 
 * SPFVertexMulticast objects in the SPF tree, along with the details of the link
...
public:
/**
 * @brief Enumeration of the possible types of SPFVertexMulticast objects.
 *
...
/**
 * @brief Construct an empty ("uninitialized") SPFVertexMulticast (Shortest Path First 
 * Vertex).
 *
 ...
 * null as is the parent SPFVertexMulticast.  The outgoing interface index is set to
 * infinity, the next hop address is set to 0.0.0.0 and the list of children
 * of the SPFVertexMulticast is initialized to empty.
 *
 * @see VertexType
 */
  SPFVertexMulticast();
...
/**
 * @brief Construct an initialized SPFVertexMulticast (Shortest Path First Vertex).
 *
 ...
 * of the SPFVertexMulticast is initialized to empty.
 *
 * @see SPFVertexMulticast::SPFVertexMulticast ()
 * @see VertexType
 * @see GlobalRoutingLSAMulticast
 * @param lsa The Link State Advertisement used for finding initial values.
 */
  SPFVertexMulticast(GlobalRoutingLSAMulticast* lsa);

/**
 * @brief Destroy an SPFVertexMulticast (Shortest Path First Vertex).
 *
 * The children vertices of the SPFVertexMulticast are recursively deleted.
 *
 * @see SPFVertexMulticast::SPFVertexMulticast ()
 */
  ~SPFVertexMulticast();
...
/**
 * @brief Get the Vertex Type field of a SPFVertexMulticast object.
 *
 * The Vertex Type describes the kind of simulation object a given SPFVertexMulticast
 * represents.
 *
 * @see VertexType
 * @returns The VertexType of the current SPFVertexMulticast object.
 */
  VertexType GetVertexType (void) const;

/**
 * @brief Set the Vertex Type field of a SPFVertexMulticast object.
 *
 * The Vertex Type describes the kind of simulation object a given SPFVertexMulticast
 * represents.
 *
 * @see VertexType
 * @param type The new VertexType for the current SPFVertexMulticast object.
 */
  void SetVertexType (VertexType type);

/**
 * @brief Get the Vertex ID field of a SPFVertexMulticast object.
 *
 * The Vertex ID uniquely identifies the simulation object a given SPFVertexMulticast
 * represents.  Typically, this is the Router ID for SPFVertexMulticast objects 
 ...
 * instantiated.
 *
 * @returns The Ipv4Address Vertex ID of the current SPFVertexMulticast object.
 */
  Ipv4Address GetVertexId (void) const;

/**
 * @brief Set the Vertex ID field of a SPFVertexMulticast object.
 *
 * The Vertex ID uniquely identifies the simulation object a given SPFVertexMulticast
 * represents.  Typically, this is the Router ID for SPFVertexMulticast objects 
 * representing routers, and comes from the Link State Advertisement of a 
 ...
 * generated value.
 *
 * @param id The new Ipv4Address Vertex ID for the current SPFVertexMulticast object.
 */
  void SetVertexId (Ipv4Address id);

/**
 * @brief Get the Global Router Link State Advertisement returned by the 
 * Global Router represented by this SPFVertexMulticast during the route discovery 
 * process.
 *
 * @see GlobalRouterMulticast
 * @see GlobalRoutingLSAMulticast
 * @see GlobalRouterMulticast::DiscoverLSAs ()
 * @returns A pointer to the GlobalRoutingLSAMulticast found by the router represented
 * by this SPFVertexMulticast object.
 */
  GlobalRoutingLSAMulticast* GetLSA (void) const;
 ...
 * process.
 *
 * @see SPFVertexMulticast::GetLSA ()
 * @see GlobalRouterMulticast
 * @see GlobalRoutingLSAMulticast
 * @see GlobalRouterMulticast::DiscoverLSAs ()
 * @warning Ownership of the LSA is transferred to the "this" SPFVertexMulticast.  You
 * must not delete the LSA after calling this method.
 * @param lsa A pointer to the GlobalRoutingLSAMulticast.
 */
  void SetLSA (GlobalRoutingLSAMulticast* lsa);

/**
 * @brief Get the distance from the root vertex to "this" SPFVertexMulticast object.
 *
 * Each router in the simulation is associated with an SPFVertexMulticast object.  When
 * calculating routes, each of these routers is, in turn, chosen as the "root"
 * of the calculation and routes to all of the other routers are eventually
 ...
 * routers in the calculation has an associated SPFVertexMulticast.
 *
 * The "Root" vertex is then the SPFVertexMulticast representing the router that is
 * having its routing tables set.  The "this" SPFVertexMulticast is the vertex to which
 ...
 *
 * @returns The distance, in hops, from the root SPFVertexMulticast to "this" SPFVertexMulticast.
 */
  uint32_t GetDistanceFromRoot (void) const;

/**
 * @brief Set the distance from the root vertex to "this" SPFVertexMulticast object.
 *
 * Each router in the simulation is associated with an SPFVertexMulticast object.  When
 * calculating routes, each of these routers is, in turn, chosen as the "root"
 * of the calculation and routes to all of the other routers are eventually
 ...
 * in question.
 *
 * @param distance The distance, in hops, from the root SPFVertexMulticast to "this"
 * SPFVertexMulticast.
 */
 ...
 * to begin forwarding packets from the root SPFVertexMulticast to "this" SPFVertexMulticast.
 *
 * Each router node in the simulation is associated with an SPFVertexMulticast object.
 ...
 *
 * The "Root" vertex is then the SPFVertexMulticast representing the router that is
 * having its routing tables set.  The "this" SPFVertexMulticast is the vertex that
 ...
 *
 * When initializing the root SPFVertexMulticast, the IP address used when forwarding
 * packets is determined by examining the Global Router Link Records of the
 * Link State Advertisement generated by the root node's GlobalRouterMulticast.  This
 ...
 * should I send a packet to the network or host represented by 'this' SPFVertexMulticast.
 *
 * @see GlobalRouterMulticast
 * @see GlobalRoutingLSAMulticast
 * @see GlobalRoutingLinkRecordMulticast
 * @param nextHop The IP address to use when forwarding packets to the host
 * or network represented by "this" SPFVertexMulticast.
 * @param id The interface index to use when forwarding packets to the host or
 * network represented by "this" SPFVertexMulticast.
 */
  void SetRootExitDirection (Ipv4Address nextHop, int32_t id = SPF_INFINITY_MULTICAST);
 ...
 * forwarding packets to the host or network represented by "this" SPFVertexMulticast.
 */
  void SetRootExitDirection (SPFVertexMulticast::NodeExit_t exit);
  /**
   ...
   */
  void MergeRootExitDirections (const SPFVertexMulticast* vertex);
  /**
   ...
   */
  void InheritAllRootExitDirections (const SPFVertexMulticast* vertex);
  /**
 ...
 *
 * This method returns a pointer to the parent node of "this" SPFVertexMulticast
 * (both of which reside in that SPF tree).
 *
 * @param i The index to one of the parents
 * @returns A pointer to the SPFVertexMulticast that is the parent of "this" SPFVertexMulticast
 * in the SPF tree.
 */
  SPFVertexMulticast* GetParent (uint32_t i = 0) const;
 ...
 * having its routing tables set and is the root of the SPF tree.
 *
 * This method sets the parent pointer of "this" SPFVertexMulticast (both of which
 * reside in that SPF tree).
 *
 * @param parent A pointer to the SPFVertexMulticast that is the parent of "this" 
 * SPFVertexMulticast* in the SPF tree.
 */
  void SetParent (SPFVertexMulticast* parent);
  /**
   ...
   */
  void MergeParent (const SPFVertexMulticast* v);

/**
 * @brief Get the number of children of "this" SPFVertexMulticast.
 *
 * Each router node in the simulation is associated with an SPFVertexMulticast object.
 * When calculating routes, each of these routers is, in turn, chosen as the 
 * "root" of the calculation and routes to all of the other routers are
 ...
 *
 * This method returns the number of children of "this" SPFVertexMulticast (which 
 * reside in the SPF tree).
 *
 * @returns The number of children of "this" SPFVertexMulticast (which reside in the
 * SPF tree).
 */
  uint32_t GetNChildren (void) const;

/**
 * @brief Get a borrowed SPFVertexMulticast pointer to the specified child of "this" 
 * SPFVertexMulticast.
 *
 * Each router node in the simulation is associated with an SPFVertexMulticast object.
 * When calculating routes, each of these routers is, in turn, chosen as the 
 ...
 * network routes available via that vertex.
 *
 * This method the number of children of "this" SPFVertexMulticast (which reside in
 * the SPF tree.
 *
 * @see SPFVertexMulticast::GetNChildren
 * @param n The index (from 0 to the number of children minus 1) of the 
 * child SPFVertexMulticast to return.
 ...
 * that object.
 * @returns A pointer to the specified child SPFVertexMulticast (which resides in the
 * SPF tree).
 */
 ...
 * SPFVertexMulticast is transferred to the "this" SPFVertexMulticast.  You must not delete the
 * (now) child SPFVertexMulticast after calling this method.
 * @param child A pointer to the SPFVertexMulticast (which resides in the SPF tree) to
 * be added to the list of children of "this" SPFVertexMulticast.
 * @returns The number of children of "this" SPFVertexMulticast after the addition of
 * the new child.
 */
  uint32_t AddChild (SPFVertexMulticast* child);
...
private:
  ...
  GlobalRoutingLSAMulticast* m_lsa; //!< Link State Advertisement
  ...
  typedef std::list<SPFVertexMulticast*> ListOfSPFVertex_t; //!< container of SPFVertexes
...
/**
 * @brief The SPFVertexMulticast copy construction is disallowed.  There's no need for
 * it and a compiler provided shallow copy would be wrong.
 * @param v object to copy from
 */
  SPFVertexMulticast (SPFVertexMulticast& v);

/**
 * @brief The SPFVertexMulticast copy assignment operator is disallowed.  There's no 
 * need for it and a compiler provided shallow copy would be wrong.
 * @param v object to copy from
 * @returns the copied object
 */
  SPFVertexMulticast& operator= (SPFVertexMulticast& v);
   ...
   */
  friend std::ostream& operator<< (std::ostream& os, const SPFVertexMulticast::ListOfSPFVertex_t& vs);
};
 ...
 * GlobalRouterMulticast interface.  The primary job of this interface is to export
 ...
 */
class GlobalRouteManagerLSDBMulticast
{
public:
/**
 ...
 * this constructor.
 */
  GlobalRouteManagerLSDBMulticast ();
 ...
 * State Database.
 *
 * The IPV4 address and the GlobalRoutingLSAMulticast given as parameters are converted
 ...
 */
  void Insert (Ipv4Address addr, GlobalRoutingLSAMulticast* lsa);
 ...
 * GlobalRoutingLSAMulticast is returned.
 *
 * @see GlobalRoutingLSAMulticast
 * @see Ipv4Address
 * @param addr The IP address associated with the LSA.  Typically the Router 
 ...
 */
  GlobalRoutingLSAMulticast* GetLSA (Ipv4Address addr) const;
/**
 * @brief Look up the Link State Advertisement associated with the given
 ...
 * ID.
 */
  GlobalRoutingLSAMulticast* GetLSAByLinkData (Ipv4Address addr) const;
 ...
 * prior to each SPF calculation to reset the state of the SPFVertexMulticast structures
 ...
 */
   ...
   */
  GlobalRoutingLSAMulticast* GetExtLSA (uint32_t index) const;
  /**
...
private:
  typedef std::map<Ipv4Address, GlobalRoutingLSAMulticast*> LSDBMap_t; //!< container of IPv4 addresses / Link State Advertisements
  typedef std::pair<Ipv4Address, GlobalRoutingLSAMulticast*> LSDBPair_t; //!< pair of IPv4 addresses / Link State Advertisements

  LSDBMap_t m_database; //!< database of IPv4 addresses / Link State Advertisements
  std::vector<GlobalRoutingLSAMulticast*> m_extdatabase; //!< database of External Link State Advertisements

/**
 * @brief GlobalRouteManagerLSDBMulticast copy construction is disallowed.  There's no 
 * need for it and a compiler provided shallow copy would be wrong.
 * @param lsdb object to copy from
 */
  GlobalRouteManagerLSDBMulticast (GlobalRouteManagerLSDBMulticast& lsdb);
 ...
 * @returns the copied object
 */
  GlobalRouteManagerLSDBMulticast& operator= (GlobalRouteManagerLSDBMulticast& lsdb);
};
 ...
 * for a GlobalRouterMulticast interface.  For those nodes, it fetches one or
 ...
 */
class GlobalRouteManagerImplMulticast
{
public:
  GlobalRouteManagerImplMulticast ();
  virtual ~GlobalRouteManagerImplMulticast ();
/**
 ...
 * from each node exporting a GlobalRouterMulticast interface.
 */
 ...
 */
  void DebugUseLsdb (GlobalRouteManagerLSDBMulticast*);
...
private:
/**
 * @brief GlobalRouteManagerImplMulticast copy construction is disallowed.
 * There's no  need for it and a compiler provided shallow copy would be 
 * wrong.
 *
 * @param srmi object to copy from
 */
  GlobalRouteManagerImplMulticast (GlobalRouteManagerImplMulticast& srmi);
 ...
 * @returns the copied object
 */
  GlobalRouteManagerImplMulticast& operator= (GlobalRouteManagerImplMulticast& srmi);

  SPFVertexMulticast* m_spfroot; //!< the root node
  GlobalRouteManagerLSDBMulticast* m_lsdb; //!< the Link State DataBase (LSDB) of the Global Route Manager
   ...
   */
  void SPFProcessStubs (SPFVertexMulticast* v);
   ...
   */
  void ProcessASExternals (SPFVertexMulticast* v, GlobalRoutingLSAMulticast* extlsa);
   ...
   */
  void SPFNext (SPFVertexMulticast* v, CandidateQueueMulticast& candidate);
   ...
   */
  int SPFNexthopCalculation (SPFVertexMulticast* v, SPFVertexMulticast* w, 
                             GlobalRoutingLinkRecordMulticast* l, uint32_t distance);
   ...
   * \param v the vertex
   */
  void SPFVertexAddParent (SPFVertexMulticast* v);
   ...
   */
  GlobalRoutingLinkRecordMulticast* SPFGetNextLink (SPFVertexMulticast* v, SPFVertexMulticast* w, 
   ...
   *
   */
  void SPFIntraAddRouter (SPFVertexMulticast* v);
   ...
   * \param v the vertex
   */
  void SPFIntraAddTransit (SPFVertexMulticast* v);
   ...
   * \param v the vertex
   */
  void SPFIntraAddStub (GlobalRoutingLinkRecordMulticast *l, SPFVertexMulticast* v);
   ...
   * \param v the vertex
   */
  void SPFAddASExternal (GlobalRoutingLSAMulticast *extlsa, SPFVertexMulticast *v);
