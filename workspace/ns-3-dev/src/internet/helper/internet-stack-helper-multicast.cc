 ...
 * "Tx" (ns3::Ipv4L3ProtocolMulticast::m_txTrace), "Rx" (ns3::Ipv4L3ProtocolMulticast::m_rxTrace) 
 * and "Drop" (ns3::Ipv4L3ProtocolMulticast::m_dropTrace) trace sources.
 *
 * The "Tx" trace is fired in a number of situations, all of which indicate that
 * a given packet is about to be sent down to a given ns3::Ipv4InterfaceMulticast.
 *
 ...
 *   the trace is hit.  See ns3::Ipv4L3ProtocolMulticast::Send.
 *
 ...
 *   gateway.  See ns3::Ipv4L3ProtocolMulticast::SendRealOut.
 *
 ...
 *   to the discovered route.  See ns3::Ipv4L3ProtocolMulticast::SendRealOut.
 *
 * The "Rx" trace is fired when a packet is passed from the device up to the
 * ns3::Ipv4L3ProtocolMulticast::Receive function.
 *
 * - In the receive function, the Ipv4InterfaceList is iterated, and if the
 *   Ipv4InterfaceMulticast corresponding to the receiving device is fount to be in the
 ...
 *
 * - In the ns3::Ipv4InterfaceMulticast::Receive function, the packet is dropped and the
 *   drop trace is hit if the interface corresponding to the receiving device
 *   is in the DOWN state.
 *
 * - Also in the ns3::Ipv4InterfaceMulticast::Receive function, the packet is dropped and
 *   the drop trace is hit if the checksum is found to be bad.
 *
 * - In ns3::Ipv4L3ProtocolMulticast::Send, an outgoing packet bound for the broadcast
 ...
 *
 * - Also in ns3::Ipv4L3ProtocolMulticast::Send, an outgoing packet destined for the 
 ...
 *
 * - In ns3::Ipv4L3ProtocolMulticast::SendRealOut, an outgoing packet being routed
 *   is dropped and the "Drop" trace is fired if the "don't fragement" bit is 
 *   set and fragmentation is available and required.
 *
 * - Also in ns3::Ipv4L3ProtocolMulticast::SendRealOut, an outgoing packet being routed
 ...
 *   if the required Ipv4InterfaceMulticast is in the DOWN state.
 *
 * - If a packet is being forwarded, and the TTL is exceeded (see
 *   ns3::Ipv4L3ProtocolMulticast::DoForward), the packet is dropped and the "Drop" trace 
 *   event is fired.
 *
...
#include "ns3/arp-l3-protocol-multicast.h"
#include "internet-stack-helper-multicast.h"
#include "ns3/ipv4-global-routing-multicast.h"
#include "ns3/ipv4-list-routing-helper-multicast.h"
#include "ns3/ipv4-static-routing-helper-multicast.h"
#include "ns3/ipv4-global-routing-helper-multicast.h"
...
NS_LOG_COMPONENT_DEFINE ("InternetStackHelperMulticast");
...
// are going to multiplex receive and transmit callbacks for all Ipv4Multicast and 
...
// Ipv4Multicast and interface pair into a corresponding Ptr<PcapFileWrapper> at the 
// callback.
//
// A complication in this situation is that the trace sources are hooked on 
// a protocol basis.  There is no trace source hooked by an Ipv4Multicast and interface
// pair.  This means that if we naively proceed to hook, say, a drop trace
// for a given Ipv4Multicast with interface 0, and then hook for Ipv4Multicast with interface 1
...
// bill.
//
typedef std::pair<Ptr<Ipv4Multicast>, uint32_t> InterfacePairIpv4;  /**< Ipv4Multicast/interface pair */
typedef std::map<InterfacePairIpv4, Ptr<PcapFileWrapper> > InterfaceFileMapIpv4;  /**< Ipv4Multicast/interface and Pcap file wrapper container */
typedef std::map<InterfacePairIpv4, Ptr<OutputStreamWrapper> > InterfaceStreamMapIpv4;  /**< Ipv4Multicast/interface and output stream container */

static InterfaceFileMapIpv4 g_interfaceFileMapIpv4; /**< A mapping of Ipv4Multicast/interface pairs to pcap files */
static InterfaceStreamMapIpv4 g_interfaceStreamMapIpv4; /**< A mapping of Ipv4Multicast/interface pairs to ascii streams */
...
InternetStackHelperMulticast::InternetStackHelperMulticast ()
  : m_routing (0),
    m_routingv6 (0),
    m_ipv4Enabled (true),
    m_ipv6Enabled (true),
...
void
InternetStackHelperMulticast::Initialize ()
{
  SetTcp ("ns3::TcpL4Protocol");
  Ipv4StaticRoutingHelperMulticast staticRouting;
  Ipv4GlobalRoutingHelperMulticast globalRouting;
  Ipv4ListRoutingHelperMulticast listRouting;
...
InternetStackHelperMulticast::InternetStackHelperMulticast (const InternetStackHelperMulticast &o)
{
...
InternetStackHelperMulticast &
InternetStackHelperMulticast::operator = (const InternetStackHelperMulticast &o)
{
  if (this == &o)
    {
      return *this;
...
void
InternetStackHelperMulticast::Reset (void)
{
  delete m_routing;
  m_routing = 0;
  delete m_routingv6;
...
void 
InternetStackHelperMulticast::SetRoutingHelper (const Ipv4RoutingHelperMulticast &routing)
{
  delete m_routing;
...
void
InternetStackHelperMulticast::SetRoutingHelper (const Ipv6RoutingHelper &routing)
{
  delete m_routingv6;
...
void
InternetStackHelperMulticast::SetIpv4StackInstall (bool enable)
{
  m_ipv4Enabled = enable;
}

void InternetStackHelperMulticast::SetIpv6StackInstall (bool enable)
{
  m_ipv6Enabled = enable;
}

void InternetStackHelperMulticast::SetIpv4ArpJitter (bool enable)
{
  m_ipv4ArpJitterEnabled = enable;
}

void InternetStackHelperMulticast::SetIpv6NsRsJitter (bool enable)
{
  m_ipv6NsRsJitterEnabled = enable;
}

int64_t
InternetStackHelperMulticast::AssignStreams (NodeContainer c, int64_t stream)
{
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      ...
      if (ipv4 != 0)
        {
          Ptr<ArpL3ProtocolMulticast> arpL3Protocol = ipv4->GetObject<ArpL3ProtocolMulticast> ();
          if (arpL3Protocol != 0)
            {
...
void
InternetStackHelperMulticast::SetTcp (const std::string tid)
{
  m_tcpFactory.SetTypeId (tid);
}

void 
InternetStackHelperMulticast::SetTcp (std::string tid, std::string n0, const AttributeValue &v0)
{
  m_tcpFactory.SetTypeId (tid);
...
void 
InternetStackHelperMulticast::Install (NodeContainer c) const
{
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Install (*i);
...
void 
InternetStackHelperMulticast::InstallAll (void) const
{
  Install (NodeContainer::GetGlobal ());
}

void
InternetStackHelperMulticast::CreateAndAggregateObjectFromTypeId (Ptr<Node> node, const std::string typeId)
{
...
void
InternetStackHelperMulticast::Install (Ptr<Node> node) const
{
  if (m_ipv4Enabled)
    {
      if (node->GetObject<Ipv4Multicast> () != 0)
        {
          NS_FATAL_ERROR ("InternetStackHelperMulticast::Install (): Aggregating " 
                          "an InternetStack to a node with an existing Ipv4Multicast object");
          return;
        }
      ...
      // Set routing
      Ptr<Ipv4Multicast> ipv4 = node->GetObject<Ipv4Multicast> ();
      Ptr<Ipv4RoutingProtocolMulticast> ipv4Routing = m_routing->Create (node);
...
void
InternetStackHelperMulticast::Install (std::string nodeName) const
{
 ...
 */
static void
Ipv4L3ProtocolRxTxSink (Ptr<const Packet> p, Ptr<Ipv4Multicast> ipv4, uint32_t interface)
{
...
bool
InternetStackHelperMulticast::PcapHooked (Ptr<Ipv4Multicast> ipv4)
{
...
void 
InternetStackHelperMulticast::EnablePcapIpv4Internal (std::string prefix, Ptr<Ipv4Multicast> ipv4, uint32_t interface, bool explicitFilename)
{
  NS_LOG_FUNCTION (prefix << ipv4 << interface);

  if (!m_ipv4Enabled)
    {
      NS_LOG_INFO ("Call to enable Ipv4Multicast pcap tracing but Ipv4Multicast not enabled");
      return;
    }
  ...
  if (!PcapHooked (ipv4))
    {
      //
      // Ptr<Ipv4Multicast> is aggregated to node and Ipv4L3ProtocolMulticast is aggregated to 
      // node so we can get to Ipv4L3ProtocolMulticast through Ipv4Multicast.
      //
      Ptr<Ipv4L3ProtocolMulticast> ipv4L3Protocol = ipv4->GetObject<Ipv4L3ProtocolMulticast> ();
      NS_ASSERT_MSG (ipv4L3Protocol, "InternetStackHelperMulticast::EnablePcapIpv4Internal(): "
      ...
      NS_ASSERT_MSG (result == true, "InternetStackHelperMulticast::EnablePcapIpv4Internal():  "
...
bool
InternetStackHelperMulticast::PcapHooked (Ptr<Ipv6> ipv6)
{
...
void 
InternetStackHelperMulticast::EnablePcapIpv6Internal (std::string prefix, Ptr<Ipv6> ipv6, uint32_t interface, bool explicitFilename)
{
  ...
  if (!PcapHooked (ipv6))
    {
      ...
      NS_ASSERT_MSG (ipv6L3Protocol, "InternetStackHelperMulticast::EnablePcapIpv6Internal(): "
      ...
      NS_ASSERT_MSG (result == true, "InternetStackHelperMulticast::EnablePcapIpv6Internal():  "
...
Ipv4L3ProtocolDropSinkWithoutContext (
  ...
  Ipv4L3ProtocolMulticast::DropReason reason, 
...
bool
InternetStackHelperMulticast::AsciiHooked (Ptr<Ipv4Multicast> ipv4)
{
...
void 
InternetStackHelperMulticast::EnableAsciiIpv4Internal (
  Ptr<OutputStreamWrapper> stream, 
  std::string prefix, 
  Ptr<Ipv4Multicast> ipv4, 
  uint32_t interface,
  bool explicitFilename)
{
  if (!m_ipv4Enabled)
    {
      NS_LOG_INFO ("Call to enable Ipv4Multicast ascii tracing but Ipv4Multicast not enabled");
      return;
    }
  ...
  if (stream == 0)
    {
      ...
      if (!AsciiHooked (ipv4))
        {
          //
          // We can use the default drop sink for the ArpL3ProtocolMulticast since it has
          // the usual signature.  We can get to the Ptr<ArpL3ProtocolMulticast> through
          // our Ptr<Ipv4Multicast> since they must both be aggregated to the same node.
          //
          Ptr<ArpL3ProtocolMulticast> arpL3Protocol = ipv4->GetObject<ArpL3ProtocolMulticast> ();
          asciiTraceHelper.HookDefaultDropSinkWithoutContext<ArpL3ProtocolMulticast> (arpL3Protocol, "Drop", theStream);

          //
          // The drop sink for the Ipv4L3ProtocolMulticast uses a different signature than
          // the default sink, so we have to cook one up for ourselves.  We can get
          // to the Ptr<Ipv4L3ProtocolMulticast> through our Ptr<Ipv4Multicast> since they must both 
          ...
          NS_ASSERT_MSG (result == true, "InternetStackHelperMulticast::EnableAsciiIpv4Internal():  "
  ...
  if (!AsciiHooked (ipv4))
    {
      ...
      //
      oss << "/NodeList/" << node->GetId () << "/$ns3::ArpL3ProtocolMulticast/Drop";
      ...
      //
      oss.str ("");
      oss << "/NodeList/" << node->GetId () << "/$ns3::Ipv4L3ProtocolMulticast/Drop";
      Config::Connect (oss.str (), MakeBoundCallback (&Ipv4L3ProtocolDropSinkWithContext, stream));
      oss.str ("");
      oss << "/NodeList/" << node->GetId () << "/$ns3::Ipv4L3ProtocolMulticast/Tx";
      Config::Connect (oss.str (), MakeBoundCallback (&Ipv4L3ProtocolTxSinkWithContext, stream));
      oss.str ("");
      oss << "/NodeList/" << node->GetId () << "/$ns3::Ipv4L3ProtocolMulticast/Rx";
...
bool
InternetStackHelperMulticast::AsciiHooked (Ptr<Ipv6> ipv6)
{
...
void 
InternetStackHelperMulticast::EnableAsciiIpv6Internal (
  Ptr<OutputStreamWrapper> stream, 
  std::string prefix, 
  Ptr<Ipv6> ipv6, 
  uint32_t interface,
      ...
      if (!AsciiHooked (ipv6))
        {
          ...
          NS_ASSERT_MSG (result == true, "InternetStackHelperMulticast::EnableAsciiIpv6Internal():  "
