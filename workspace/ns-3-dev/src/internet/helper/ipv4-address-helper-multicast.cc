...
#include "ipv4-address-helper-multicast.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("Ipv4AddressHelperMulticast");

Ipv4AddressHelperMulticast::Ipv4AddressHelperMulticast () 
{
...
void
Ipv4AddressHelperMulticast::SetBase (
  const Ipv4Address network, 
  const Ipv4Mask mask,
  const Ipv4Address address)
{
...
//
  NS_ASSERT_MSG ((m_network & ~m_mask) == 0,
                 "Ipv4AddressHelperMulticast::SetBase(): Inconsistent network and mask");
...
//
  ...
  NS_ASSERT_MSG (m_shift <= 32,
                 "Ipv4AddressHelperMulticast::SetBase(): Unreasonable address length");
...
Ipv4Address
Ipv4AddressHelperMulticast::NewAddress (void)
{
//
...
//
  NS_ASSERT_MSG (m_address <= m_max,
                 "Ipv4AddressHelperMulticast::NewAddress(): Address overflow");
...
Ipv4Address
Ipv4AddressHelperMulticast::NewNetwork (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  ++m_network;
  m_address = m_base;
...
Ipv4AddressHelperMulticast::Assign (const NetDeviceContainer &c)
{
  NS_LOG_FUNCTION_NOARGS ();
  Ipv4InterfaceContainerMulticast retval;
  for (uint32_t i = 0; i < c.GetN (); ++i) {
      Ptr<NetDevice> device = c.Get (i);

      Ptr<Node> node = device->GetNode ();
      NS_ASSERT_MSG (node, "Ipv4AddressHelperMulticast::Assign(): NetDevice is not not associated "
                     "with any node -> fail");

      Ptr<Ipv4Multicast> ipv4 = node->GetObject<Ipv4Multicast> ();
      NS_ASSERT_MSG (ipv4, "Ipv4AddressHelperMulticast::Assign(): NetDevice is associated"
      ...
      NS_ASSERT_MSG (interface >= 0, "Ipv4AddressHelperMulticast::Assign(): "
...
uint32_t
Ipv4AddressHelperMulticast::NumAddressBits (uint32_t maskbits) const
{
  NS_LOG_FUNCTION_NOARGS ();
  ...
  NS_ASSERT_MSG (false, "Ipv4AddressHelperMulticast::NumAddressBits(): Bad Mask");
  return 0;
}
