...
#ifndef IPV4_INTERFACE_CONTAINER_MULTICAST_H
#define IPV4_INTERFACE_CONTAINER_MULTICAST_H
...
/**
 * \brief holds a vector of std::pair of Ptr<Ipv4Multicast> and interface index.
 *
 * Typically ns-3 Ipv4Interfaces are installed on devices using an Ipv4Multicast address
 ...
 * Ptr<Ipv4Multicast>.  It makes sure that an interface exists on the node for the 
 ...
 * the Ptr<Ipv4Multicast> and the interface index to a std::pair and adds them to a 
 ...
 * an identical index into the Ipv4InterfaceContainerMulticast that has a std::pair with
 * the Ptr<Ipv4Multicast> and interface index you need to play with the interface.
 *
 * @see Ipv4AddressHelper
 * @see Ipv4Multicast
 */
class Ipv4InterfaceContainerMulticast
{
public:
  /**
   * \brief Container Const Iterator for pairs of Ipv4Multicast smart pointer / Interface Index.
   */
  typedef std::vector<std::pair<Ptr<Ipv4Multicast>, uint32_t> >::const_iterator Iterator;

  /**
   * Create an empty Ipv4InterfaceContainerMulticast.
   */
  Ipv4InterfaceContainerMulticast ();

  /**
   * Concatenate the entries in the other container with ours.
   * \param other container
   */
  void Add (Ipv4InterfaceContainerMulticast other);
   ...
   *
   * \code
   *   Ipv4InterfaceContainerMulticast::Iterator i;
  ...
  /**
   * \returns the number of Ptr<Ipv4Multicast> and interface pairs stored in this 
   * Ipv4InterfaceContainerMulticast.
   *
   * Pairs can be retrieved from the container in two ways.  First,
   * directly by an index into the container, and second, using an iterator.
   ...
   *     {
   *       std::pair<Ptr<Ipv4Multicast>, uint32_t> pair = container.Get (i);
   *       method (pair.first, pair.second);  // use the pair
   *     }
   * \endcode
   *
   ...
   *
   * \param ipv4 pointer to Ipv4Multicast object
   * \param interface interface index of the Ipv4Interface to add to the container
   *
   * @see Ipv4InterfaceContainerMulticast
   */
  void Add (Ptr<Ipv4Multicast> ipv4, uint32_t interface);

  /**
   * Manually add an entry to the container consisting of a previously composed 
   * entry std::pair.
   *
   * \param ipInterfacePair the pair of a pointer to Ipv4Multicast object and interface index of the Ipv4Interface to add to the container
   *
   * @see Ipv4InterfaceContainerMulticast
   */
  void Add (std::pair<Ptr<Ipv4Multicast>, uint32_t> ipInterfacePair);

  /**
   * Manually add an entry to the container consisting of the individual parts
   * of an entry std::pair.
   *
   * \param ipv4Name std:string referring to the saved name of an Ipv4Multicast Object that
  ...
  /**
   * Get the std::pair of an Ptr<Ipv4Multicast> and interface stored at the location 
   * specified by the index.
   *
   * \param i the index of the entry to retrieve.
   * \return the Ipv4InterfaceContainerMulticast
   *
   * @see Ipv4InterfaceContainerMulticast
   */
  std::pair<Ptr<Ipv4Multicast>, uint32_t> Get (uint32_t i) const;

private:
  /**
   * \brief Container for pairs of Ipv4Multicast smart pointer / Interface Index.
   */
  typedef std::vector<std::pair<Ptr<Ipv4Multicast>,uint32_t> > InterfaceVector;
