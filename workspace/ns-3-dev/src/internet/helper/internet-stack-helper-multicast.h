...
#ifndef INTERNET_STACK_HELPER_MULTICAST_H
#define INTERNET_STACK_HELPER_MULTICAST_H
 ...
 *  - ns3::ArpL3ProtocolMulticast
 *  - ns3::Ipv4L3ProtocolMulticast
 *  - ns3::Icmpv4L4Protocol
 *  - ns3::UdpL4Protocol
 *  - a TCP based on the TCP factory provided
 *  - a PacketSocketFactory
 *  - Ipv4Multicast routing (a list routing object and a static routing object)
 */
class InternetStackHelperMulticast : public PcapHelperForIpv4Multicast, public PcapHelperForIpv6, 
                            public AsciiTraceHelperForIpv4Multicast, public AsciiTraceHelperForIpv6
{
public:
  /**
   * Create a new InternetStackHelperMulticast which uses a mix of static routing
   * and global routing by default. The static routing protocol 
   * (ns3::Ipv4StaticRouting) and the global routing protocol are
   * stored in an ns3::Ipv4ListRoutingMulticast protocol with priorities 0, and -10
   * by default. If you wish to use different priorites and different
   * routing protocols, you need to use an adhoc ns3::Ipv4RoutingHelperMulticast, 
   * such as ns3::OlsrHelper
   */
  InternetStackHelperMulticast(void);

  /**
   * Destroy the InternetStackHelperMulticast
   */
  virtual ~InternetStackHelperMulticast(void);

  /**
   * \brief Copy constructor
   */
  InternetStackHelperMulticast (const InternetStackHelperMulticast &);
   ...
   * an object of type ns3::Ipv4RoutingProtocolMulticast per node. This routing
   * object is then associated to a single ns3::Ipv4Multicast object through its 
   * ns3::Ipv4Multicast::SetRoutingProtocol.
   */
  void SetRoutingHelper (const Ipv4RoutingHelperMulticast &routing);
  ...
  /**
   * Aggregate implementations of the ns3::Ipv4Multicast, ns3::Ipv6, ns3::Udp, and ns3::Tcp classes
   * onto the provided node.  This method will assert if called on a node that 
   * already has an Ipv4Multicast object aggregated to it.
   * 
   ...
   * ns3::Ipv4Multicast, ns3::Ipv6, ns3::Udp, and, ns3::Tcp classes.  The program will assert 
   * if this method is called on a container with a node that already has
   * an Ipv4Multicast object aggregated to it.
   * 
...
private:
   ...
   * @param ipv4 Ptr to the Ipv4Multicast interface on which you want to enable tracing.
   * @param interface Interface ID on the Ipv4Multicast on which you want to enable tracing.
   ...
   */
  const Ipv4RoutingHelperMulticast *m_routing;
   ...
   */
  bool PcapHooked (Ptr<Ipv4Multicast> ipv4);
   ...
   * \returns true if a hook is found
   */
  bool AsciiHooked (Ptr<Ipv4Multicast> ipv4);
